...
2. 经SAE的基向量$k$的激活为$a_k^{(1)}=a_k(s^+)$， $a_k^{(0)}=a_k(s^−)$。$a_k(s)$为该句所有 token 上基向量 $k$ 激活的最大值。每个基向量对每个句子的个体潜在效应为个体潜在效应为:$τ=a_k^{(1)}-a_k^{(0)}$，在该特定数据集上有$EALE_k=\sum_{i=1}^Nτ_k(s_i)$。通过计算每个基向量的$EALE_k$进行敏感性预过滤，并保留绝对值超过第75百分位数的那些。
...
3. 在SAE后的向量空间，定义二元触发器$Z_k(s) = \mathbb{I}[a_k(s) \geq \theta_k]$。
    ...
    **必要概率 (PN)**。
    ...
    特征表征置信s度 (FRC)。 我们用调和平均将两个因果概率结合，以惩罚仅充分或仅必要的向量：
