...
在使用ai时，也要利用分层的思想，因为ai的注意力只会延对话进行单线索地叠加。以上面的第二类ai工具为例，可以并行多个会话，其中一个维护着从阅读到各个任务板块的项目主线，一个专用于某个板块内部，一个用于一些微调，另一个用于分析硬件配置与算力问题之类。另外，如果有一系列实验性的串联的任务，建议分别在并列的目录下建立完整的数据流、控制流，虽然会带来不少冗余，但是可以解决串联执行中的混乱问题，更加易于维护和阅读结果与过程。
...
prompt中的表达，也尽量用出处为给agent提供的知识中的术语。
...
此外，要善于利用AI Agent写各种脚本来实现许多自己可能想要实现的但不会实现、没有实现过的东西，来辅助实验流程的自动化。比如可以等待一个进程执行完后接着执行下一个进程。但需要注意的是，所等待的进程应该是**实际干活的进程**，莫错当**shell进程**。
